...
# Agent traffic key (#266): the key CoPaw presents as Bearer.
if ! talon secrets list 2>/dev/null | grep -q copaw-main-talon-key; then
  talon secrets set copaw-main-talon-key "${TALON_AGENT_KEY:-talon-gw-copaw-001}" 2>/dev/null || true
fi
