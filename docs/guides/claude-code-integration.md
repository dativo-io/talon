...
- `agents/codex.talon.yaml` — the `codex` agent for Codex CLI (see the [Codex guide](codex-cli-integration.md)). To serve both agents from one `talon serve`, set `agents_dir` in `talon.config.yaml`: discovery (#267, shipped) loads every file named exactly `agent.talon.yaml` under it, so move the codex agent to `agents/codex/agent.talon.yaml` and provision both agents' keys. Without `agents_dir`, `talon serve` runs the single default `agent.talon.yaml`.
