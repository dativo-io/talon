...
| **Session budget lookup** (#198) | `BenchmarkSessionBudgetLookup` | `internal/gateway` | The per-request cost the session budget check adds to the hot path: one caller-scoped tuple read (`GetByExternal`) + one stage-count read on a 200-session SQLite store. Budget: **< 1 ms** per op (measured ~0.03 ms on the reference laptop). |
