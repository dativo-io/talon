...
**The native runtime is still single-policy.** Fleet Operations v1 lands in stages: `agents_dir` currently governs the gateway identity registry (each discovered agent's key routes and attributes to its own agent); the agent-first native surfaces (`talon run --agent`, server run requests, triggers, `talon costs --agent`) still govern by the one `default_policy` file until the multi-agent native runtime PR closes #267 — naming any OTHER agent errors explicitly (`unknown agent … (#267)`) instead of silently running under the loaded policy (#290; see LIMITATIONS.md §8).
