...
| `POLICY_DENIED_ROUTING` | `deny` | `policy_evaluation` | routing/provider reason; gateway: `Model <name> not in agent allowlist`, `Model <name> is blocked for this agent`, `Data tier N exceeds agent restriction` | Request blocked by model routing policy. | Select an allowed model/provider for the data tier. |
