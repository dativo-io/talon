...
- **Response-PII actions other than `allow` buffer whole streams.** With `response_pii_action` set to `block`/`redact`/`warn`, the full SSE stream is buffered before release: time-to-first-token becomes total generation time — unusable for interactive coding. The coding-agents pack therefore defaults coding agents to `response_pii_action: allow` (input-side scanning still applies). Also raise `request_timeout` for coding agents: the 120s default hard-cuts long generations and Codex aborts idle streams ~300s (#217). The header wait defaults to `request_timeout` (tunable via `response_header_timeout`), so slow-TTFB non-streaming calls are no longer cut at `connect_timeout` (#230, fixed).
...
## 8. Single-Agent-Policy Boundary on NATIVE surfaces (until the multi-agent runtime PR closes #267)

**The gateway fleet is multi-agent; the native runtime is not yet.** With `agents_dir` set in `talon.config.yaml`, the gateway identity registry holds every discovered `agent.talon.yaml` — each agent's key routes and attributes to its own agent (#267, Fleet Operations v1 PR-1). The agent-first NATIVE surfaces (`talon run --agent`, triggers, the server run path, `talon costs --agent` offline) still govern by the ONE policy selected via `TALON_DEFAULT_POLICY` (default `agent.talon.yaml`) or `--policy`, until the multi-agent native runtime lands (compiled per-agent runtime bundles — engine, scanner, router — resolved from the shared catalog).

- **Unknown agents fail loudly, never silently as the default.** A native run, trigger, or server run request naming an agent other than the loaded policy's `agent.name` errors with `unknown agent … (#267)` instead of running the wrong name under the loaded policy's budgets and tools. *(Backed by `TestRunFromTrigger_usesDefaultPolicyPath`.)*
- **Budget answers never guess.** With a running gateway, `/v1/costs/budget` answers `budget_source: unknown_agent` for an unregistered agent; offline, `talon costs --agent <other>` reports no denominator with an explicit until-#267 note — neither ever reports the default agent file's caps as another agent's budget (#288). *(Backed by `TestCostsBudget_RuntimeResolvedContract`.)*
- **Native multi-agent selection today is per process:** point `TALON_DEFAULT_POLICY` at a different agent file per run process. The multi-agent native runtime PR replaces this with catalog resolution over the same discovered fleet.
