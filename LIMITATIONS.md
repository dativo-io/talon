...
## 8. Single-Agent-Policy Boundary (until agents_dir, #267)

**Exactly one agent policy is loaded per process.** Several surfaces read agent-first (`talon doctor`, `talon costs --agent`, the native runner, the server policy path) but until agents_dir discovery lands (#267) they all govern by the ONE policy selected via `TALON_DEFAULT_POLICY` (default `agent.talon.yaml`) or `--policy`. The gateway registry likewise holds that one keyed agent (#266).
...
- **Multi-agent selection today is per process:** point `TALON_DEFAULT_POLICY` at a different agent file per serve/run process. #267 replaces this with agents_dir discovery over the same registry.
