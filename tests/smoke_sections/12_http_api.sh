...
GWEOF
  ...
  # The agent key (identity registry projection) authenticates /v1/evidence (#266)
  local agent_key="${TALON_AGENT_KEY:-talon-api-tenant-001}"
  ...
  if [[ -n "$run1_session" ]]; then
    ...
    local complete_code; complete_code="$(curl -s -o /dev/null -w '%{http_code}' -X POST -H "Authorization: Bearer $agent_key" "http://127.0.0.1:8080/v1/sessions/$run1_session/complete")"
